...
// Returns size of the QF in bytes.
uint64_t qf_usefile(QF* qf, const char* filename, int flag)
{
	...
	qf->runtimedata = (qfruntime *)calloc(sizeof(qfruntime), 1);
	if (qf->runtimedata == NULL) {
		perror("GQF ERROR: Couldn't allocate memory for runtime data.");
		return 0;
	}
	qf->runtimedata->f_info.fd = open(filename, open_flag);
	if (qf->runtimedata->f_info.fd < 0) {
		perror("GQF ERROR: Couldn't open file.");
		return 0;
	}

	ret = fstat (qf->runtimedata->f_info.fd, &sb);
	if ( ret < 0) {
		perror ("GQF ERROR: fstat");
		return 0;
	}

	if (!S_ISREG (sb.st_mode)) {
		fprintf (stderr, "GQF ERROR: %s is not a file.\n", filename);
		return 0;
	}
...
bool qf_closefile(QF* qf)
{
	...
	// qf_sync_counters(qf); // not needed since we are only reading. for some reason doesnt handle case.
